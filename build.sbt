...
lazy val `woken-validation` =
  project
    ...
    .settings(
      Seq(
        (Runtime / mainClass) := Some("ch.chuv.lren.woken.validation.Main"),
        ...
        (Compile / unmanagedResources / includeFilter) := "*.xml" || "*.conf",
        (Test / unmanagedResources / includeFilter) := "*.json",
        // Use the customMergeStrategy in your settings
        (assembly / assemblyMergeStrategy) := customMergeStrategy,
        (assembly / assemblyJarName) := "woken-validation-all.jar"
      )
    )
...
lazy val commonSettings =
  Seq(
    ...
    (ThisBuild / description) := "Cross validation module for Woken",
